...
import ru.gorevmichael.files.domain.usecases.ExportJsonUseCase
...
class CreateCustomSignConfigViewModel(
    ...
    private val exportJsonUseCase: ExportJsonUseCase = ExportJsonUseCase()
) : ViewModel() {

    private val _uiState = MutableStateFlow(CreateCustomSignConfigUiState())
    private var dataToSave: Pair<String, SignConfig>? = null
    val uiState = _uiState.asStateFlow()

    fun onNameChanged(value: String) =
        _uiState.update { it.copy(name = value, isSaved = false, error = null) }

    fun onAChanged(value: String) =
        _uiState.update { it.copy(a = value, isSaved = false, error = null) }

    fun onBChanged(value: String) =
        _uiState.update { it.copy(b = value, isSaved = false, error = null) }

    fun onPChanged(value: String) =
        _uiState.update { it.copy(p = value, isSaved = false, error = null) }

    fun onGxChanged(value: String) =
        _uiState.update { it.copy(gx = value, isSaved = false, error = null) }

    fun onGyChanged(value: String) =
        _uiState.update { it.copy(gy = value, isSaved = false, error = null) }

    fun onNChanged(value: String) =
        _uiState.update { it.copy(n = value, isSaved = false, error = null) }

    fun saveConfig() {
        viewModelScope.launch {
            collectDataToSave()
            if (dataToSave == null) {
                _uiState.update { it.copy(error = "Ошибка сохранения: не удалось получить данные!") }
                return@launch
            } else {
                try {
                    saveCustomSignConfigUseCase(dataToSave!!.first, dataToSave!!.second)
                    _uiState.update { it.copy(isSaved = true, error = null) }
                } catch (e: Exception) {
                    e.printStackTrace()
                    _uiState.update { it.copy(isSaved = false, error = e.message) }
                }
            }
        }
    }
    ...
    fun exportConfig() {
        viewModelScope.launch {
            collectDataToSave()
            if (dataToSave == null) {
                _uiState.update { it.copy(error = "Ошибка экспорта: не удалось получить данные!") }
                return@launch
            } else {
                try {
                    val name = dataToSave!!.first
                    val signConfig = dataToSave!!.second
                    val content = """
                        {
                          "name": "$name",
                    ...
                    """.trimIndent()
                    exportJsonUseCase(name, content)
                    _uiState.update { it.copy(isSaved = true, error = null) }
                } catch (e: Exception) {
                    e.printStackTrace()
                    _uiState.update { it.copy(isSaved = false, error = e.message) }
    ...
    private fun collectDataToSave() {
        ...
        try {
            ...
            dataToSave = state.name to signConfig
        } catch (e: Exception) {
